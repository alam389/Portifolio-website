...
// Grainient — animated grain-gradient backdrop, ported from React Bits
// (https://reactbits.dev, MIT). Adapted for this repo: TypeScript, Tailwind
// container classes instead of the companion CSS file, and a
// prefers-reduced-motion path that renders one static frame instead of
// animating. Renders via WebGL2 (ogl); pauses offscreen and on hidden tabs.
...
// Keep renderer/program alive across re-renders so the props effect can
...
export default function Grainient({
  ...
  // Effect 1: build WebGL context once; pause offscreen / hidden tab.
  useEffect(() => {
    ...
    // Reduced motion: keep the gradient + grain, skip the animation loop.
  ...
  useEffect(() => {
    ...
    // Reduced-motion path never runs the loop — repaint once so prop
    // changes still show.
